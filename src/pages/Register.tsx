...
export const Register: React.FC = () => {
  ...
  useEffect(() => {
    if (!authLoading && user && userData) {
      // User has complete profile, redirect based on role
  ...
  // Check if current user is a Google user without Firestore document
  useEffect(() => {
    const checkGoogleUser = async () => {
      if (user && !userData && !authLoading) {
        // User is logged in via Google but doesn't have Firestore document
  ...
  const validateForm = () => {
    ...
    // Validate password - required for non-Google users, optional for Google users
    if (!isGoogleUser && !user) {
      ...
      // For Google users who want to set a password (optional)
      if (formData.password.length < 6) {
        setError('Password must be at least 6 characters');
        return false;
      }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If user is already logged in via Google, just create the Firestore document
      if (user) {
        ...
        // Force a page reload to refresh AuthContext and redirect
        if (role === 'admin') {
          window.location.href = '/admin';
        } else {
          window.location.href = '/student';
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (userDoc) {
        // User already exists with complete profile, redirect based on role
        ...
        // User signed in with Google but no Firestore document
        // Keep them signed in and show the form to complete registration
        setIsGoogleUser(true);
        setFormData(prev => ({
          ...prev,
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="font-display font-extrabold text-4xl text-[#3D4852] tracking-tight">
            {isGoogleUser || user ? 'Complete Registration' : 'Create Account'}
          </h1>
        ...
        <Card className="p-10">
          <form onSubmit={handleRegister} className="space-y-5">
            {(isGoogleUser || user) && (
              <div className="p-4 rounded-2xl bg-green-50 text-green-700 text-sm shadow-[inset_3px_3px_6px_rgba(100,200,100,0.1),inset_-3px_-3px_6px_rgba(255,255,255,0.5)]">
                <div className="flex items-center gap-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  Signed in with Google! Complete your profile below. Password is optional - you can use Google to login.
                </div>
              </div>
            )}
            ...
            <Input
              ...
              onChange={handleChange}
              required
              disabled={isGoogleUser || !!user}
            />

            <div className="grid grid-cols-2 gap-4">
              <Input
                label={isGoogleUser || user ? "Password (Optional)" : "Password"}
                type="password"
                name="password"
                placeholder="Create password"
                value={formData.password}
                onChange={handleChange}
                required={!isGoogleUser && !user}
              />

              <Input
                label={isGoogleUser || user ? "Confirm (Optional)" : "Confirm Password"}
                type="password"
          ...
          {/* Only show Google button for non-Google users */}
          {!isGoogleUser && !user && (
            <>
              {/* Divider */}
