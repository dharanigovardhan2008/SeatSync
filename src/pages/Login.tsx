...
import { loginWithEmail, loginWithGoogle, getUserDocument } from '@/lib/firebase';
...
export const Login: React.FC = () => {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!userDoc) {
        // User created account but never completed registration
      ...
      const userData = userDoc as { id: string; role: string };
      const role = userEmail === 'palerugopi2008@gmail.com' ? 'admin' : userData.role;
      
      // Use window.location to force full page reload and refresh AuthContext
      if (role === 'admin') {
        window.location.href = '/admin';
      } else {
        window.location.href = '/student';
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (userDoc) {
        ...
        // Keep them signed in and redirect to register to complete profile
        window.location.href = '/register';
      }
    } catch (err: unknown) {
